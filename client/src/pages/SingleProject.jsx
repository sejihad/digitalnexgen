 ...
 <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold font-roboto tracking-tight text-primaryText dark:text-white mb-6">
            ...
            <div className="w-full">
              <div className="columns-1 sm:columns-2 md:columns-3 gap-4 [column-fill:_balance]">
                {project.images.map((image, idx) => (
                  ...
                  >
                    <img src={image} alt={`img-${idx}`} className="w-full h-auto object-cover transition-transform duration-500 group-hover:scale-[1.02]" />
  ...
  <div className="bg-white rounded-2xl shadow-sm p-6 dark:bg-white/10 dark:shadow-lg border border-gray-100 dark:border-white/20">
    ...
    <div className="mt-6 text-sm text-gray-500">
      ...
      <div>
        <strong>Delivered:</strong> {new Date(project.createdAt).toLocaleDateString()}
      </div>
    </div>
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
          <div className="relative max-w-[90vw] max-h-[90vh] w-full flex items-center justify-center">
            <button onClick={() => setSelectedImage(null)} className="absolute top-4 right-4 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full shadow-lg backdrop-blur-sm z-20 transition" aria-label="Close preview">✕</button>
            <img src={selectedImage} alt="Preview" className="rounded-lg shadow-xl max-w-full max-h-[90vh] object-contain transition-transform duration-300" style={{ transform: `scale(${zoom})` }} onClick={() => setZoom(prev => (prev === 1 ? 2 : 1))} />
          </div>
        </div>
      )}
    </div>
